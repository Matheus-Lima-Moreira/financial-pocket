...
export default function ProfileAvatarUpload({ profile }: ProfileAvatarUploadProps) {
  ...
  const currentAvatarUrl = preview || profile.avatar || "";
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      const result = await userAvatarUploadAction(profile.id, formData);
  ...
  const handleRemove = async () => {
    ...
    try {
      const result = await userAvatarRemoveAction(profile.id);
        ...
        <div className="relative shrink-0">
          <Avatar className="size-20 border-2 border-zinc-300">
            ...
            <AvatarFallback className="bg-muted text-base">
              {profile.name ? getInitials(profile.name) : <User className="size-6" />}
            </AvatarFallback>
          </Avatar>
          {showRemoveButton && (
            <Tooltip>
